...
def worker_beat():
    from ercc.models import SiteConfiguration
    from ercc.models import SchemaCollector
...
def _sendmail(receiver, title, body):
    from ercc.utils.email import sendmail as sendmail_direct
...
def _build_glossary_report(report_id):
    from ercc.models import GlossaryReport
...
def _xls_loader(loader_id):
    from ercc.models import XlsLoaderHistory
...
def _build_glossary_model_report(report_id):
    from ercc.models import GlossaryModelReport
...
def _collect_schema(project_id, schema_collector_id):
    ...
    from ercc.ercc_bps.schema.forms import SchemaCollector
    from ercc.ercc_bps.schema.schema_collectors import DbmsSchemaCollector
