...
def parsley_kwargs(field, kwargs):
    ...
    for vali in field.validators:
        ...
        if not isinstance(vali, Optional) \
        ...
        if isinstance(vali, Email):
            _email_kwargs(new_kwargs, message)
...
def _email_kwargs(kwargs, message):
    kwargs[u'data-parsley-type'] = u'email'
    if message:
        kwargs[u'data-parsley-type'] = message
...
def _length_kwargs(kwargs, vali, message):
    default_number = -1

    if vali.max != default_number and vali.min != default_number:
        kwargs[u'data-parsley-length'] = u'[' + str(vali.min) + u',' + str(vali.max) + u']'
        if message:
            kwargs[u'data-parsley-length-message'] = message
    else:
        if vali.max == default_number:
            kwargs[u'data-parsley-minlength'] = str(vali.min)
            if message:
                kwargs[u'data-parsley-minlength-message'] = message
        if vali.min == default_number:
            kwargs[u'data-parsley-maxlength'] = str(vali.max)
            if message:
                kwargs[u'data-parsley-maxlength-message'] = message


def _number_range_kwargs(kwargs, vali, message):
    kwargs[u'data-parsley-range'] = u'[' + str(vali.min) + u',' + str(vali.max) + u']'
    if message:
        kwargs[u'data-parsley-range-message'] = message


def _input_required_kwargs(kwargs, message):
    kwargs[u'data-parsley-required'] = u'true'
    if message:
        kwargs[u'data-parsley-required-message'] = message


def _regexp_kwargs(kwargs, vali, message):
    ...
    if message:
        kwargs[u'data-parsley-pattern-message'] = message
